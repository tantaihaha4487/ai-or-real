...
export function TimerBar({ secondsLeft, durationSeconds = 10, className }: { secondsLeft: number; durationSeconds?: number; className?: string }) {
  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <Progress value={progress} className='h-3 rounded-full bg-white/[0.08]' indicatorClassName={tone} />
    </div>
  )
}
